---
title: Два редактора, один документ. История совместного редактирования текста
...
<iframe
  src="https://www.youtube.com/embed/ztk_H1UOWso"
  title="YouTube video player" frameborder="0"
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
  allowfullscreen></iframe>

## Проблематика
Представьте себе документ, который редактируют несколько человек одновременно. У меня был такой опыт, и я не могу сказать, что он был достаточно приятным. А почему так происходит? Неужели настолько сложно синхронизировать один документ даже между двумя редакторами? Оказывается, да, две конкурентные операции способны привести к совершенно разным результатам. Данная статья посвящена популярным подходам к совместному редактированию текста в течение последних 20 лет, а также неординарным архитектурным решениям и компромиссам, на которые приходится идти.
...
## Наивный подход
Все операции над текстом проходят через центрального арбитра, который формирует их общий порядок и применяет операции в соответствии с этим порядком. Все преобразования над итоговым документом происходят только на арбитре, а клиенты лишь получают снапшот, который перезаписывает все их локальные изменения.

Очевидно, что это самый примитивный и самый неэффективный вариант, так как снапшот, отправляемый таким арбитром переопределяет все локальные изменения, а сам арбитр выступает бутылочным горлышком при большом числе редакторов.

![Центральный арбитр](./arbitre.png)

## Operational Transformation
Суть этого подхода заключается в переопределении операции изменения в соответствии с локальным контекстом через функцию трансформации. Под локальным контекстом понимается множество операций, которые применены локально к версии документа у одного из клиентов. К примеру, если требуется вставить символ на позицию 1, а вместе с этим изменением конкурентно пришла операция удаления символа на позиции 0, то функция трансформации преобразует операцию вставки, поставив новый символ на позицию 0.
...
Несмотря на свои недостатки, WOOT по сей день используется в проекте XWiki Concerto, представляющий собой P2P версию XWiki[^Xwiki].
...
### Logoot
Logoot вышел в 2009 году, как ответ на слабости WOOT в виде неограниченного роста tombstone-ов и дорогой рекурсии при конфликтной вставке.
...
Для вставки нового символа, Logoot анализирует идентификаторы соседей P и Q и арифметически вычисляет свой собственный.
...
3. Если пространства для вставки нет ни на одном уровне, идентификатор записывает левого "родителя" и удлиняется на один уровень глубже, где место есть всегда. Это означает, что список увеличивается на одну запись.

Нетрудно догадаться, что список идентификаторов представляет собой дерево, каждый новый уровень которого является "приближением" более высокой точности между двумя соседними символами.

![Logoot1](./logoot1.png)
![Logoot2](./logoot2.png)

За счёт такого подхода к вставке Logoot лишён необходимости рекурсивного обхода конфликтного участка, достаточно лишь вставки в упорядоченную структуру, которая выполняется за `O(log n)`, отсюда, собственно, и название (**Log**-oot).

Кроме того, архитектурно Logoot меньше завязан на tombstone-ах, и более гибок в плане их устранения, что позволяет эффективнее оптимизировать память[^Logoot].

### LSEQ
Логическое продолжение Logoot, предлагающее другой алгоритм выбора позиции на уровне. Во-первых, LSEQ удваивает диапазон, из которого выбираются числа, для каждого нового уровня. Во-вторых, предлагаются стратегии `boundary+` и `boundary-`, оптимизирующие различный характер редактирования текста (более частая вставка в конец для `boundary+` и более частая вставка в начало для `boundary-`). Сами стратегии отвечают за то, к какой из границ диапазона будет склоняться выбор числа на уровне. В реальных системах данные стратегии чередуются на уровнях, чтобы сгладить эффект обоих стилей редактирования[^Lseq].
...
[^TP2]: https://arxiv.org/pdf/2207.02502
[^Jupiter]: https://arxiv.org/pdf/1708.04754 
[^Tombstones]: https://inria.hal.science/inria-00109039v1/document
[^Woot]: https://inria.hal.science/inria-00108523v1/document
[^Xwiki]: https://www.researchgate.net/profile/Pascal-Molli/publication/40497572_XWiki_Concerto_A_P2P_Wiki_System_Supporting_Disconnected_Work/links/0deec5208fddb6fde7000000/XWiki-Concerto-A-P2P-Wiki-System-Supporting-Disconnected-Work.pdf
...
[^Egwalker]: https://arxiv.org/pdf/2409.14252
